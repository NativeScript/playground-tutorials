...
* **a.** Open your `app/views/login/login.xml` file and paste in the following code.
...
* **a.** Open your `app/views/login/login.xml` file and replaces its contents with the following code, which adds a new `<FlexboxLayout>` component.
...
<Page>
	...
	<FlexboxLayout>
        ...
        autocapitalizationType="none" class="input input-border"/>
			<TextField class="input input-border" hint="Password" secure="true" 
...
* **a.** Open your app’s `app/views/login/login.xml` file, and replace the code currently there with the following code, which adds a new `<Image>` component.
