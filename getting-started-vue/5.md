...
* **b.** In the `<script>` section, modify the `onItemTap` method:
...
* **c.** Create the `onTodoCircleTap` method to handle events for the small tappable icon. 
    * Method catches the index of the tapped item.
    * Method moves elements from the `todos` array to the `dones` array. Use `splice()` to avoid leaving holes in the array and `unshift()` to make sure that recently completed tasks are shown on top.
    * When the method moves a task to the `dones` array, use the `activeTabIndex` to switch the tab to the completed tab.
