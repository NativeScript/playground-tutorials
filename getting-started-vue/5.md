...
* **c.** Remember the Playground and the Preview apps you downloaded on your device, so you could test your code on device? Try your progress out and check if everything works as expected at this point.
...
> **TIP:**
> Your `onTodoCircleTap` method is currently called when a barely visible empty space is tapped. To test if it works as expected, you might want to temporarily tweak the respective `<Label>` component. For example: `<Label col="0" v-on:tap="onTodoCircleTap(todo)" text="TEST" />`  
