...
* **d.** Configure the positioning of the components within the nested grid (the input area).
    * Set the `<TextField>` to inhabit the first column.
    * Set the `<Button>` to inhabit the second column.

```HTML
<TextField col="0" v-model="textFieldValue" hint="Enter text..." />
<Button col="1" text="+" @tap="onButtonTap" />
```

* **e.** Clean up sample code from the `<ListView>`&mdash;remove the `<FlexboxLayout>` component and add a new `<Label>` in its place. Remove the height setting. Set `textWrap="true"` for the `<Label>` and retain the `:text="country.name"` binding to continue showing the list of countries.
            ...
            <TabViewItem title="To Do">
                ...
                <GridLayout rows="auto, auto, *">
                    ...
                    <GridLayout row="1" columns="3*, *" rows="auto">
                        ...
                        <TextField col="0" v-model="textFieldValue"
                            hint="Enter text..." />
                        <Button col="1" text="+" @tap="onButtonTap" />
                    </GridLayout>
...
Your `onButtonTap` method should resemble this sample:
...
* **d.** Remember the Playground and the Preview apps you downloaded on your device, so you could test your code on device? Try your progress out and check if everything works as expected at this point.

* **e.** Return to the `<template>` block and modify the `<ListView>` section. Insert another `<GridLayout>` within the `v-template` section in the list view. Clean up the `<GridLayout>` and set it to consist of two columns.
...
* **g.** Modify the `<TextField>` so that pressing `Return` on the keyboard is tied to its own method. Use the `@returnPress` property.

```HTML
<TextField col="0" row="0" v-model="textFieldValue" hint="Type new task..." @returnPress="onReturnPress" />
```

* **h.** Create a new `onReturnPress` method that checks if the entered string is not empty and based on the result acts as if the **+** button was pressed.
