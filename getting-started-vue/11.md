...
* **b.** In the `methods` section of the `<script>` block, add a `onPageLoaded` method at the top and create a simple `platform.isIOS` check. You will use it to manage iOS-specific changes as soon as the app screen loads.
...
* **c.** Call this method in the `@loaded` event for the `<Page>` element in your `<template>` block.
